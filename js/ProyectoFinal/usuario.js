function all() {
    $.ajax({
        url: 'http://localhost:9000/electiva2/api/usuario', // Reemplaza con la URL de tu API
        method: 'GET',
...
function findById(id){
    $.ajax({
        url: 'http://localhost:9000/electiva2/api/usuario/'+id, // Reemplaza con la URL de tu API
        method: 'GET',
        success: function(data) {
            $("#id").val(data.id);
...
function save(){    
    var nuevoPersonaId = parseInt($("#personaId").val(), 10);
    if (isNaN(nuevoPersonaId)) {
        alert('Persona ID no es un número válido');
        return;
    } 
...
function update(){
    ...
    // Enviar datos al servidor mediante AJAX
    $.ajax({
        url: 'http://localhost:9000/electiva2/api/usuario/'+$("#id").val(), // Reemplaza con la URL de tu API
