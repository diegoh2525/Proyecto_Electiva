function all() {
    $.ajax({
        url: 'http://localhost:9000/electiva2/api/moduloVista', // Reemplaza con la URL de tu API
        method: 'GET',
...
function findById(id){
    $.ajax({
        url: 'http://localhost:9000/electiva2/api/moduloVista/'+id, // Reemplaza con la URL de tu API
        method: 'GET',
        success: function(data) {
            $("#id").val(data.id);
            $("#moduloId").val(data.moduloId.id);
            $("#vistaId").val(data.vistaId.id);
...
function save() {    
    ...
    var nuevoVistaId = parseInt($("#vistaId").val(), 10);
    if (isNaN(nuevoVistaId)) {
        alert('Vista ID no es un número válido');
        return;
    } 
...
function update(){
    ...
    // Enviar datos al servidor mediante AJAX
    $.ajax({
        url: 'http://localhost:9000/electiva2/api/moduloVista/'+$("#id").val(), // Reemplaza con la URL de tu API
